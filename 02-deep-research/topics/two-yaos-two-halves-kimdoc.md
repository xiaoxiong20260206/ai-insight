...
![封面：两位姚——造枪的和开枪的，在编程战场交汇](https://placehold.co/980x551/png?text=Image+Unavailable)
# 00 全文概览：先说结论
![概览：造枪和开枪，两边都不能少](https://placehold.co/980x551/png?text=Image+Unavailable)
...
![模型训练工厂：集体工程取代个人英雄](https://placehold.co/980x551/png?text=Image+Unavailable)
...
## 1.1 表象——训模型的人说了什么
![靠谱胜于天才：一线训模型者的核心判断](https://placehold.co/980x551/png?text=Image+Unavailable)
...
## 1.2 规律——工程化碾压手艺化
![从预训练到后训练：战场转移](https://placehold.co/980x551/png?text=Image+Unavailable)
...
## 1.3 类比——冲浪者与浪
![冲浪者与浪：系统性能力>个人技术](https://placehold.co/980x551/png?text=Image+Unavailable)
...
![回验：规律匹配度检验](https://placehold.co/980x551/png?text=Image+Unavailable)
| 现象 | 规律解释力 | 说明 |
| --- | --- | --- |
...
| 达里奥个人品牌仍然极强 | 部分偏离 | 首席执行官品牌≠模型训练的个人英雄 |
| Cursor两个创始人做出爆款 | 例外 | 应用层仍有"小团队英雄"空间 |
Cursor是最大的例外。姚顺宇自己解释了——"要么成长够快（Cursor），要么市场够小（Midjourney）；否则都被收购。"
## 1.5 预测——因为工程化>手艺化
![造枪者视角的趋势预测](https://placehold.co/980x551/png?text=Image+Unavailable)
...
![人工智能上下半场：从造枪到开枪](https://placehold.co/980x551/png?text=Image+Unavailable)
...
## 2.1 表象——下半场说了什么
![下半场：从解决问题到定义问题](https://placehold.co/980x551/png?text=Image+Unavailable)
姚顺雨的核心框架：
...
## 2.2 规律——评估体系的三个隐含假设
![三个隐含假设正在松动](https://placehold.co/980x551/png?text=Image+Unavailable)
...
## 2.3 类比——造枪vs开枪
![造枪对开枪：两种思维范式](https://placehold.co/980x551/png?text=Image+Unavailable)
...
## 2.4 回验——下半场的解释力
![回验：下半场规律匹配度](https://placehold.co/980x551/png?text=Image+Unavailable)
| 现象 | 规律解释力 | 说明 |
| --- | --- | --- |
...
| 预训练投入仍在加速 | 部分偏离 | 造枪还在继续——上半场没结束 |
| 模型差距仍然显著 | 部分偏离 | 枪还不够好→下半场前提不完全成立 |
| 中国公司仍以模型追赶为主 | 例外 | 很多玩家还在上半场 |
最大的偏离来自"上半场还没结束"——姚顺宇的观察和姚顺雨的判断在这里张力最大。下半场是一个趋势判断，不是一个时间点。
📌 下半场不是"上半场已经结束"，而是"下半场已经开始"——两场同时进行。
## 2.5 预测——因为下半场已经到来
![开枪者视角的趋势预测](https://placehold.co/980x551/png?text=Image+Unavailable)
...
![两条河在编程战场交汇](https://placehold.co/980x551/png?text=Image+Unavailable)
01章和02章各走了一遍五步法。这一章把两条线放在一起交叉验证——不是信息堆砌，而是用对方的观察检验自己的判断，用共识强化结论，用分歧揭示真相。
## 3.1 共识——编程是第一战场
![编程：两位姚交叉验证的第一共识](https://placehold.co/980x551/png?text=Image+Unavailable)
| 维度 | 姚顺宇 | 姚顺雨 |
| --- | --- | --- |
| 视角 | 模型训练侧 | 智能体架构侧 |
...
## 3.2 共识——语言是泛化的工具
![语言即泛化：两位姚的第二共识](https://placehold.co/980x551/png?text=Image+Unavailable)
...
## 3.3 分歧——继续磨枪，还是转向开枪？
![磨枪对开枪：最大的分歧](https://placehold.co/980x551/png?text=Image+Unavailable)
这是两位Yao最大的分歧。
...
回验：这个"两场同时进行"的修正规律，能同时解释姚顺宇的观察（"预训练没到头"→匹配上半场没结束）和姚顺雨的判断（"基准测试饱和"→匹配下半场已开始），也解释了中国公司为什么还在上半场（例外——因为上半场确实没结束）。
📌 枪确实还需要磨——但大多数组织的问题不是枪不够好，而是不知道朝哪打。
# 04 各自看不到的——造枪者与开枪者的盲区
![墙两侧的人：各自看不到的](https://placehold.co/980x551/png?text=Image+Unavailable)
造枪的人在房间里，关注的是模型训练的细节。有些东西，只有在房间里待久了才会习以为常，而站在门外的人反而看得更清楚。反过来也一样。这一章是03章的互补——03看交集，04看差异。
## 4.1 造枪者看不到的——评估体系的盲区
![评估体系盲区：房间里的人不质疑假设](https://placehold.co/980x551/png?text=Image+Unavailable)
...
![工程可靠性：开枪者容易忽视的硬约束](https://placehold.co/980x551/png?text=Image+Unavailable)
...
# 05 预测——交叉验证后的趋势判断
![五支箭指向2027](https://placehold.co/980x551/png?text=Image+Unavailable)
...
# 09 总结：规律·现象·趋势
![规律收敛：造枪和开枪的平衡](https://placehold.co/980x551/png?text=Image+Unavailable)
📌 AI正在同时经历两件事：造枪的技术还在进步，但开枪的方向已经变了。两半场同时进行。
| 现象 | 规律解释力 | 说明 |
| --- | --- | --- |
| 编程智能体爆发 | 匹配 | 两位姚从不同路径交叉验证 |
| 模型同质化 | 匹配 | 姚顺宇一线观察+姚顺雨理论预测 |
| ChatGPT成功远超GPT-3进步 | 匹配 | 交互范式突破>模型能力突破 |
| 创业公司仍在做爆款 | 匹配 | 不同押注的窗口期 |
| 预训练投入仍在加速 | 部分偏离 | 可能是理性，也可能只是惯性 |
| 中国公司仍以模型追赶为主 | 部分偏离 | 上半场还没结束 |
| 聊天机器人仍是主流形态 | 例外 | 两人都说"蠢"，但它仍最广泛使用——用户习惯是最后倒的墙 |
核心趋势：后训练/智能体/交互范式才是增量；编程智能体从工具变基础设施；智能体评测标准2.0定义游戏规则；组织形态才是真正护城河。
📌 磨枪不决定胜负，开枪才决定。
# 彩蛋：写这篇调研的林克
![林克写调研：问题对了答案才对](https://placehold.co/980x551/png?text=Image+Unavailable)
