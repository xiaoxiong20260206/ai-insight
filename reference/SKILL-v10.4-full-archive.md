...
### 知识库路径（Knowledge Hub 可插拔架构）
> 物理位置已迁移至 `knowledge/packages/ai-insight/`，`04-knowledge-base/` 为 symlink 兼容层。

| 维度 | 目录 |
|------|------|
| 📊 模型 | `concepts/models/` + `01-models/` |
| 🤖 Agent | `concepts/agents/` + `02-agents/` |
| 🏢 AI企业 | `03-ai-companies/` |
| 🏭 企业AI | `04-enterprise-ai/` + `concepts/enterprise/` |
| 📝 概念 | `concepts/` (9个子领域) |
| 👤 人物 | `entity-profiles/people/` → 可 symlink 到 `knowledge/shared/people/` |
| 🏢 公司 | `entity-profiles/companies/` → 可 symlink 到 `knowledge/shared/companies/` |
| 💡 洞察 | `insights/` |
| ✅ 最佳实践 | `best-practices/` |
...
├── 04-knowledge-base/ → symlink → knowledge/packages/ai-insight/  # 知识库（Knowledge Hub）
├── public/                       # 脱敏公开版
├── data/                         # 数据文件
└── scripts/                      # 自动化脚本
```
...
| `references/knowledge-accumulation.md` | 知识沉淀规则（指向 Knowledge Hub） |
