...
### 2026-06-30: W26周报全面视觉重构（对齐 qingshuang-research-style v3.2）

**背景**：W26周报与日报视觉标准严重脱节——无卡片感、板块无区分色、stat-card无颜色、表格原生样式、侧边栏过宽。用户在当天反馈"远不如日报效果好"，触发全面重构。

---

#### 问题1：CSS分层冲突 — 零散追加补丁被基础层覆盖（P0）
- **根因**：历次优化都是向 `ai-insight-custom.css` 末尾追加 CSS 块，但基础 CSS 层（`daily-report-v3.css`）里的同名选择器用了内联 style/更高优先级，导致追加规则无效。
- **修复**：重写 `ai-insight-custom.css`，删除零散补丁，整合为一个 v4.0 系统，关键属性统一加 `!important`，并放在文件末尾保证层叠优先级最高。
- **教训①**：CSS 补丁积累超过 3 轮就必须整理合并，零散追加的 CSS 无法保证优先级，最终效果不可预测。

#### 问题2：stat-card 彩色规则被自己的裸选择器覆盖（P1）
- **根因**：删除裸 `.stat-value { color: xxx }` 规则时，把带父选择器的规则也误删成了 5 个无 `{}` 的残缺选择器，写在一行里，整个规则块失效。
- **修复**：补全 `.stat-card.stat-success .stat-value { color: #059669 !important; }` 等 5 条完整规则，放在文件末尾兜底。
- **教训②**：删 CSS 规则时必须同时检查同一块内的关联规则，不能只删"看起来多余的那一条"。

#### 问题3：板块事件表格信号列被截断（P0）
- **根因 1**：`.table-wrap { overflow: hidden }` 覆盖了 `overflow-x: auto`，表格内容溢出后直接截断。
- **根因 2**：`table-layout: fixed` + `width:100%` 但列宽百分比计算在 content-inner 有 padding 时不准确，维度列挤占信号列。
- **修复**：恢复 `overflow-x: auto !important`，改用 `table-layout: auto`，维度列 `white-space: nowrap; width: 1%`（收缩到最小），信号列自然获得剩余所有空间。
- **教训③**：表格列宽优先用 `table-layout: auto + nowrap/normal` 组合，而不是固定百分比。固定百分比在多层容器嵌套时难以准确计算。

#### 问题4：日报索引标题全是"AI洞察日报"（P1）
- **根因**：`render_daily_index` 直接读 JSON 的 `title` 字段，而每天的 `title` 统一写死为"AI洞察日报"。
- **修复**：取 `keywords.split(",")[0]` 的第一个关键词作为标题，有区分度，读者一眼看出每天核心事件。
- **教训④**：当 JSON 字段值语义重复时，优先用更有区分度的字段（keywords 比 title 更有信息量）。

#### 问题5：stat-card 全局删除（用户确认无价值）
- 概览区和各板块的统计卡片（stats-grid）在实际阅读中意义不大，占版面、数字来源不明确。
- **修复**：`render_overview` 和 `render_section` 均删除 `stats_html` 渲染逻辑，同时从 `REQUIRED_CLASSES` 移除 `stat-card`（否则 HTML 校验失败）。

---

#### 本次建立的视觉标准（写入 gen_weekly_html.py 作为永久规范）

| 组件 | 标准 |
|------|------|
| Header | 深色渐变背景（#0f172a→#1e1b4b）+ 毛玻璃 badge + 白色渐变标题 36px/800 |
| doc-chapter-label | pill 胶囊 10px/700/uppercase，板块用板块色 |
| 板块 h2 | 22px/700，板块主题色（绿/蓝/紫/橙/红） |
| 板块分隔 | margin-top:48px + border-top:2px 板块色15%淡色 |
| news-card | 白色卡片 + 4px 左色条 + hover 上浮 200ms |
| board-table | table-layout:auto，维度列 nowrap，信号列自由换行，overflow-x:auto |
| callout strong | 800字重 + 各类型对应深色 |
| 日报索引 | flex布局，日期列 72px，标题取keywords第一词 |
| 词汇表 | board-table 紫色主题，出处列有数据才显示 |
| narrative blocks | 序号小标题（01/02/03）+ 彩色 border-left |
| stat-card | 已删除，不再使用 |
| 页面背景 | #F8FAFB + mesh gradient |
| 正文 | 15px/1.75 行高/#57534E |

---

_更新于 2026-06-30 · v3.6 · W26全面视觉重构 + qingshuang-research-style v3.2 对齐 + 5条新教训_
