...
## 子技能 8: 国内信源（微信公众号 + Web搜索）

委托 `wechat-articles` + `quark-search` 技能处理，统一覆盖微信公众号、Web通用搜索。

**微信搜索流程**（4步法：账号搜索→话题搜索→质量筛选→自检）：
1. 先用 `wechat-articles` 技能搜索公众号文章（账号搜索+话题搜索）
2. 再用 `quark-search` 技能补充国内Web搜索

```
use_skill(skill_name="wechat-articles", reason="AI日报微信搜索")
use_skill(skill_name="quark-search", reason="AI日报国内补充搜索")
```
