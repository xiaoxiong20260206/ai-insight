...
> ⚠️ 只有18条需要Agent自觉遵守。其余校验已内置到脚本。

### #0. 时间窗口铁律 — 最高优先级，所有内容生成必须遵守

**日报**：只收录 N-1日08:00 ~ N日08:00 之间的新闻  
**周报**：只收录本周一00:00 ~ 周日23:59 之间的新闻

- ❌ **绝对禁止**：加入搜索结果之外的条目（即使模型"知道"是真实新闻）
- ❌ **绝对禁止**：把旧新闻（超出时间窗口）以当期日期写入
- ❌ **绝对禁止**：URL 含有比日报/周报日期早 3 天以上的日期（视为旧文章充数）
- ✅ **验证**：日报Step2开始前，先列出Step1搜索到的全部原始条目；周报Step2开始前，先从7天日报JSON提取所有news条目

质量门 v13.0 已对以上情况做 URL 嵌入日期二次验证（硬阻断）。
...
### #35 部署前必须通过全链路自检（2026-06-30）

**日报** `deploy_daily.sh` 自检链：
1. JSON Schema 验证 → 阻断
2. 质量门 22项 → 阻断（硬性失败）
3. HTML 展示验证 → 阻断
4. 首页完整性 `verify_homepage.py` → 阻断

**周报** `deploy_weekly.sh` 自检链：
1. JSON Schema 验证 → 阻断
2. HTML 展示验证 → 阻断
3. 外部版敏感词检查 → 阻断
4. 首页完整性 `verify_homepage.py` → 阻断

任何一项自检失败 = 禁止部署。修复后重新自检直到全通过。

_更新于 2026-06-30 · v3.8 · P0#35 部署前全链路自检_
