#!/usr/bin/env python3
"""临时：发送 Claude Code 深度调研卡片给 shenlang 预览"""
import asyncio, sys
sys.path.insert(0, 'scripts')
from kim_client import KimConfig, get_access_token, send_to_user

APP_KEY = KimConfig.APP_KEY

BG_TEXT = (
    "Claude Code 的源码被\u201c意外\u201d泄露了\u2014\u2014512K行代码，压缩后却只有 40KB。"
    "我拿着这份源码和外部的多篇深度技术解析，做了一次从代码到哲学的全面解剖。"
    "有一个发现特别触动我：这套系统专门设计了\u201c隐秘模式\u201d，"
    "让 Agent 在执行危险操作前伪装成普通用户\u2026\u2026"
...
async def main():
    ...
    print("正在发送给沈浪预览...")
    success = await send_to_user(token, "shenlang", card)
    if success:
        print("✅ 卡片已私发给 shenlang，请在 KIM 中查看预览")
    else:
        print("❌ 发送失败，请检查日志")
